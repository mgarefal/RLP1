...
from wtforms import StringField, SubmitField, TextAreaField, BooleanField, SelectField, DateTimeField, SelectMultipleField, HiddenField, IntegerField,URLField, EmailField
from wtforms.validators import DataRequired, Email, Length, EqualTo, ValidationError, Optional, Email, URL
...
class NewActivityForm(FlaskForm):
    ...
    activity_type = StringField('Activity Type', validators=[Length(max=5)])

    activity_metadata = TextAreaField(label="Activity Metadata (JSON)", validators=[Optional()])
...
class AppConfigurationForm(FlaskForm):
    ...
    board = SelectField('Board', choices=[('arduino:avr:uno', 'arduino:avr:uno'), ('esp8266:esp8266:nodemcuv2', 'esp8266:esp8266:nodemcuv2'), ('option3', 'Option 3')],validators=[DataRequired()])
...
class T1_GeneralMetaForm(FlaskForm):
    title = StringField('Title', validators=[DataRequired()])
    description = StringField('Description', validators=[DataRequired()])
    terms_of_service_url = URLField('Terms of Service URL', validators=[DataRequired(), URL()])
    contact_email = EmailField('Contact Email', validators=[DataRequired(), Email()])
    license = StringField('License', validators=[DataRequired()])
    license_url = URLField('License URL', validators=[DataRequired(), URL()])
    submit = SubmitField('Submit')      

class t2_binding_metadata_form(FlaskForm):
    basePath = StringField('Base Path', validators=[DataRequired()])
    apis = IntegerField('APIs', validators=[DataRequired()])
    models = IntegerField('Models', validators=[DataRequired()])
