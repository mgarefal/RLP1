...
bool redButtonState = HIGH;    // Current state of the red button
...
void setup() {
  ...
  // Activate the internal pull-ups for the buttons
...
void loop() {
  // Read the state of the yellow button (LOW when pressed, HIGH when released)
  ...
  // Read the state of the red button (LOW when pressed, HIGH when released)
  bool newRedButtonState = digitalRead(redButtonPin);

  // Check if the red button is pressed and released to stop the blink sequence
  if (newRedButtonState == LOW && redButtonState == HIGH) {
    delay(20); // Debounce delay
    newRedButtonState = digitalRead(redButtonPin);
    if (newRedButtonState == LOW) {
      blinkSequenceActive = false;
      turnOffLEDs();
    }
  }

  // Store the current state of the red button
