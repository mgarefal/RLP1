...
def remote_lab1_test():
    ...
    if request.method == 'POST':
        ...
        if sketch_id:
            ...
            SendxAPI('User Updated Sketch' ,statementType="UserAction",sketchCreated=str(sketch.article_body), sketchTitle=str(sketch.article_title))
        else:
            # Create new sketch
            ...
            SendxAPI('User Created Sketch',statementType="UserAction",sketchCreated=str(sketch.article_body), sketchTitle=str(sketch.article_title))
        db.session.commit()
...
def get_activity_data(activity_id):    
    ...
    if activity:    
        activity_data_list=[activity_id,activity.activity_title,activity.activity_body,activity.activity_metadata]
    else:    
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, password):
            if user.usergroup=="admin":
                ...
                SendxAPI("User Login",statementType="UserAction")                                
                return redirect(url_for("root"))
            else:
                BookingSystem=get_config_bookingsystem()
                if BookingSystem:
...
def logout():
    ...
    if Logout_Flag==False:
        ...
        #InitiateMaintenance()    
        SendxAPI("User Logout",statementType="UserAction")        
        session.clear()   
        logout_user()
...
def activate_activity(activity_id):
    ...
    if activity.activity_type == "NodeRED":
        SendxAPI('Attempted Activity' ,statementType="UserAction")        
    ...
    flash("Title:" + str(activity.activity_title) + "-" + str(activity.activity_body),"middle_footer")
    SendxAPI('User Attempted Activity' ,statementType="UserAction")    
    return redirect(url_for("root"))
...
def reset_activity():
    ...
    SendxAPI('User Reset Activity' ,statementType="UserAction")
...
def check_code_chatgpt(ActivityMicroActivitiesDescsSTR,UserMicroActivitiesDescsSTR):
    check_results=[None,None]
    ActiveActivity = get_ActiveActivity()
    if ActiveActivity==-1:
        flash('No active Activity.', 'success')
        return('-1')
    ...
    if activity:    
        try:
            #prompt= "The user has to write the code for the exercise with title:" + activity.activity_title + " and with these instructions: " + activity.activity_body + "the code the user uploaded is the following:"  + UserSketch.last_sketch + " tell me that the uploaded code is free of syntax errors and if it alligns to the exercise instructions. The answer must be in 3 lines max."
            prompt = f"""The user has to write the code for the exercise with title: {activity.activity_title}
and with these instructions: {activity.activity_body} The user has done the following actions: {UserMicroActivitiesDescsSTR}.
Expected actions are: {ActivityMicroActivitiesDescsSTR}. the code is: {UserSketch.last_sketch} 
Tell me if the uploaded code is free of syntax errors and if it aligns with the instructions.
Reply in 3 lines max and in json format according the following
reply:, results:,  passed: Passed or Failed, percentage_correct : 0 to 100, fail_reason: reason or null 
"""
            
            check_results[0]=prompt
            check_results[1]=chat(prompt)
            print("###### Chat Prompt: " + check_results[0] + "###########") 
            print("###### Chat Reply:  " + check_results[1] + "###########")
            return check_results

        except Exception as e:
            # Log the error for debugging purposes
            print(f"Error in check_code_chatgpt: {e}")
            return 'ErrorProcessing'
...
def submit_activity(activity_id):
    ...
    SendxAPI('User Submited Activity' ,statementType="UserAction")
    ActivityMicroActivitiesDescsSTR = ', '.join(Activity_MicroActs)
    UserMicroActivitiesDescsSTR= ', '.join(UserActivities)
    Result=CheckResults(Activity_MicroActs, UserActivities)
    if get_DebugLevel() > 0:
        print("***********" + Result + "*********")    
    #CodeResults=check_code()
    CodeResults=check_code_chatgpt(ActivityMicroActivitiesDescsSTR, UserMicroActivitiesDescsSTR)
    flash(CodeResults[0],"success")
    JSONReply = json.loads(CodeResults[1])
    flash(JSONReply["reply"],"success")
       
    AIResultPassed=JSONReply["results"]["passed"]
    AIPrompt=CodeResults[0]
    AIReply=JSONReply["reply"]
    ActivityResult=JSONReply["results"]["passed"]
    ActivityPercent=JSONReply["results"]["percentage_correct"]
    ResultReason=JSONReply["results"]["fail_reason"]
    SendxAPI("User " + AIResultPassed + " Activity",AIPrompt=AIPrompt,AIReply=AIReply,ActivityResult=ActivityResult, ActivityPercent=ActivityPercent,ResultReason=ResultReason)
    #if CodeResults==-1:
...
def activity_status():
    ...
    if ActiveActivity==-1:
        ...
        SendxAPI('Activity Status Report', statementType="UserAction", StatusReport="No Active Activity")
        return redirect(url_for("root"))
        ...
        ChatReply=CheckWithChatGPT(activity,UserMicroActivitiesDescs,ActivityMicroActivitiesDescs)
        flash(ChatReply[0])
        flash(ChatReply[1])    
        if config.ShadowController==True:
            #flash('Shadow Controller is True')
        ...
        StatusReport='The AI prompt was:' + ChatReply[0] + ' and the AI reply was:' + ChatReply[1]
        SendxAPI('Activity Status Report',statementType="UserAction",StatusReport=StatusReport)
...
def CheckWithChatGPT(activity,UserMicroActivitiesDescs,ActivityMicroActivitiesDescs):
    mychat=[None,None] 
    UserLastSketch = UserLastSketchUploaded.query.first()
    ActivityMicroActivitiesDescsSTR = ', '.join(ActivityMicroActivitiesDescs)
    UserMicroActivitiesDescsSTR= ', '.join(UserMicroActivitiesDescs)
    mychat[0]="I have this activity with title:" + activity.activity_title + " and activity description: " + activity.activity_body + " the user should do these actions according to the scenario: " + ActivityMicroActivitiesDescsSTR + " and has done the following actions " + UserMicroActivitiesDescsSTR + ". Tell me if the student did what is expected in max 3 lines (200 chars)."
    #print(prompt)
    mychat[1]=chat(mychat[0])
    return mychat
...
def new_article():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        db.session.commit()
        SendxAPI('User Created Sketch ',statementType="UserAction",sketchCreated=str(article.article_body), sketchTitle=str(article.article_title))
...
def new_activity():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            activity_metadata = form.activity_metadata.data
            ...
            try:
                db.session.commit()
                new_activity = Activities.query.filter_by(activity_title=activity_title,activity_body=activity_body).order_by(Activities.id.desc()).first()
                flash(f"Η δραστηριότητα με τίτλο {activity.activity_title} δημιουργήθηκε με επιτυχία", "success")
                SendxAPI('User Created Activity' ,statementType="TeacherAction", activity_id=new_activity.id)
                return redirect(url_for("root"))
            except Exception as e:
...
def delete_article(article_id):
    ...
    if article:
        ...
        db.session.commit()
        SendxAPI('User Deleted Sketch'  ,statementType="UserAction",sketchCreated=str(article.article_body), sketchTitle=str(article.article_title))
        flash("The sketch deleted", "success")

        return redirect(url_for("root"))

    flash("Sketch was not found.", "warning")
...
def delete_activity(activity_id):
    ...
    if activity:
        ...
        SendxAPI('User Deleted Activity ' ,statementType="TeacherAction",activity_id=activity_id)        
        db.session.delete(activity)
        db.session.commit()
        flash("Activity deleted succesfully..", "success")        
        return redirect(url_for("root"))
...
def compile_sketch(article_id):
    ...
    if article:
        ...
        SendxAPI('User Compiled Sketch',statementType="UserAction", sketchTitle=str(article.article_title),sketchUse=str(article.article_body))        
        return render_template("full_article.html", article=article)
...
def upload_sketch(article_id):
    ...
    if article:
       ...
       #     ArdCLIcommand="arduino-cli upload -b arduino:avr:uno -p /dev/ttyACM1 " + sketchDir
        ...
        SendxAPI('User uploaded Sketch', statementType="UserAction", sketchTitle=str(article.article_title),sketchUse=str(article.article_body))
        
        return render_template("full_article.html", article=article)

    flash("Το σχέδιο δε βρέθηκε.", "warning")
    SendxAPI('No Sketch found ',statementType="UserAction",sketchTitle=str(article.article_title))
    return redirect(url_for("root"))
...
def compile_upload2():
    ...
    if article:
        ...
        stdout_message = remove_ansi_escape_sequences(compile_result.stdout.decode().strip())
        stderr_message = remove_ansi_escape_sequences(compile_result.stderr.decode().strip())
        print('####### xAPI data #######')
        print(str(article.article_body))
        print(str(stdout_message))
        print(str(stderr_message))
        SendxAPI('User Compiled Sketch',statementType="UserAction", sketchUse=str(article.article_body),compileResult=str(stdout_message),compileError=str(stderr_message),sketchTitle=str(article.article_title))
        print("###### Compile #######")
        ...
        if compile_result.returncode == 0:
            ...
            if upload_result:
                ...
                SendxAPI('User Uploaded Sketch',statementType="UserAction",uploadResult=str(upload_result),sketchUse=str(article.article_body), sketchTitle=str(article.article_title))
                ...
                SendxAPI('User Failed Upload' ,statementType="UserAction",sketchTitle=str(article.article_title),sketchUse=str(article.article_body))                
                return jsonify(success=False, redirect_url=url_for('remote_lab1_test'))
        else:
            readable_error = stderr_message.replace("\n", "<br>")
            flash(f"Compilation failed:<br>{readable_error}", "danger")
            print("####### Compile Error ######")            
            SendxAPI('(Compile & Upload) Compile Failed',statementType="UserAction", compileError=str(readable_error),sketchTitle=str(article.article_title))
            
            return jsonify(success=False, redirect_url=url_for('remote_lab1_test'))
    else:
        flash("Sketch not found", "warning")
        SendxAPI('(Compile & Upload) Sketch Not Found',statementType="UserAction", sketchTitle=str(article.article_title))
        return jsonify(success=False, redirect_url=url_for('remote_lab1_test'))
...
def upload_sketch2(article):
    ...
    stdout_message = remove_ansi_escape_sequences(upload_result.stdout.decode().strip())
    stderr_message = remove_ansi_escape_sequences(upload_result.stderr.decode().strip())

    # Check if the upload succeeded
    if upload_result.returncode == 0:
        flash(stdout_message, "info")  # Show CLI output from upload
        UserActivities.append("UploadSketch")
        SendxAPI('(Compile & Upload) User Uploaded '+ str(article.article_title),statementType="UserAction",uploadResult=str(stdout_message),sketchUse=str(article.article_body))
        print("###### Upload #######")
        print("##### stdout_message ######")
        print(stdout_message)
        print("##### stderr_message ######")
        ...
        readable_error = stderr_message.replace("\n", "<br>")
        flash(f"Compilation failed:<br>{readable_error}", "danger")   
        SendxAPI('(Compile & Upload) Upload Failed',statementType="UserAction",uploadError=str(stderr_message),sketchUse=str(article.article_body),sketchTitle=str(article.article_title))
...
def edit_article(article_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        db.session.commit()
        SendxAPI('User Updated Sketch',statementType="UserAction",sketchUse=str(article.article_body),sketchTitle=str(article.article_title))
...
def edit_activity(activity_id):
    ...
    activity = Activities.query.filter_by(id=activity_id, author=current_user).first_or_404()
    form = NewActivityForm(activity_title=activity.activity_title, activity_body=activity.activity_body,activity_ctrl_sketch=activity.activity_ctrl_sketch,activity_microacts=activity.activity_microacts,activity_ml_model=activity.activity_ml_model, activity_type=activity.activity_type, activity_metadata=activity.activity_metadata)
    microActivities=MicroActivity.query.all()
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        activity.activity_metadata=form.activity_metadata.data
        if form.activity_image.data:
            try:
                image_file = image_save(form.activity_image.data, 'activities_images', (640, 360))
            except:
        ...
        db.session.commit()
        SendxAPI('User Updated Activity',statementType="TeacherAction",activity_id=activity_id)
...
def launch():
    ...
    launch_data_storage = get_launch_data_storage()    
    message_launch = ExtendedFlaskMessageLaunch(flask_request, tool_conf, launch_data_storage=launch_data_storage,requests_session=requests_session)
...
def configure(launch_id, difficulty):
    ...
    requests_session.verify = False      
    launch_data_storage = get_launch_data_storage()
    message_launch = ExtendedFlaskMessageLaunch.from_cache(launch_id, flask_request, tool_conf,launch_data_storage=launch_data_storage,requests_session=requests_session)
...
def SendxAPI(What_Data, PIN_Status=None, statementType=None,sketchCreated=None,sketchUse=None,compileResult=None,compileError=None,uploadResult=None, uploadError=None,sketchTitle=None,StatusReport=None,ActivityResult=None, ActivityPercent=None, ResultReason=None,AIPrompt=None,AIReply=None, activity_id=None):
    ...
    if activity_id is not None:
        ...
        activityID=get_ActiveActivity()
    #print(ActiveActivity)
    #print(statementType)
        if ActiveActivity==-1:    
            activityTitle="N/A"
            ActivityDesc="N/A"
            ActivityMetadata="N/A"
    
    activityData=get_activity_data(activityID)
    activityTitle=activityData[1]
    ActivityDesc=activityData[2]
    ActivityMetadata=str(activityData[3])
    print("##########" + activityTitle + " | " + ActivityDesc + " | " + ActivityMetadata)
    ...
    if get_Use_xAPI():
        ...
        mylrs.SendStatement(CurrentUser, CurrentUserEmail,What_Data,activityTitle,PIN_Status,LRS_session_id,RL_Title,statementType,sketchCreated,sketchUse,compileResult,compileError,uploadResult, uploadError,sketchTitle,StatusReport,ActivityDesc,ActivityResult, ActivityPercent, ResultReason, AIPrompt, AIReply,ActivityMetadata)    
...
# API endpoint to return the pin mapping
@app.route("/api/pins_data", methods=["GET"])
@requires_auth
def get_pins_data():
    raw_data = GetControlerResult_SerialComm()
    parts = raw_data.split('|')[1:]  # skip "Pin States"
    pins_data = {}
    for part in parts:
        if ':' in part:
            ...
            try:
                ...
                pass  # leave it as a string if not an int
            pins_data[key.strip()] = value
    return jsonify(pins_data)

# API endpoint to turn on/off check boxes
@app.route('/api/checkbox/<checkbox_id>/<action>', methods=["GET"])
@requires_auth
def api_handle_checkbox(checkbox_id, action):
    try:
        clientSocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        clientSocket.connect(("127.0.0.1", 9090))
        ...
        # Log & xAPI
        if ActiveActivity != -1:
            label = actuators[checkbox_number - 1]
            status = "ON" if action == 'on' else "OFF"
            UserActivities.append(f"{label} {status}")
            SendxAPI(f"{label} {status}", statementType="UserAction")

        clientSocket.close()
        return jsonify({
            "message": f"Checkbox {checkbox_id} turned {action.upper()}",
            "status": "success"
        })
...
def chat(user_message):
    try:
        ...
        key_path = os.path.join(BASE_DIR, "api.key")
        
        with open(key_path, "r") as f:
            myapi_key = f.read().strip()
        client = openai.OpenAI(api_key= myapi_key)
...
@app.route('/chatwithAssistant', methods=['POST'])
def chatwithAssistant():
    ...
    if "status" in user_message:
        reply = "Sure! Opening the activity status page..."
        return jsonify({'reply': reply, 'redirect': '/activity_status/'})
    if "logout" in user_message or "bye" in user_message or "log out" in user_message:
        reply = "Sure! logging you out..."
        return jsonify({'reply': reply, 'redirect': '/logout/'})
    if "chat" in user_message or "chatgpt" in user_message:
        reply = "Sure! Opening chatGPT interface"
        return jsonify({'reply': reply, 'redirect': '/chat_page'}) 
    if "ask" in user_message or "ask gpt" in user_message:
        reply = "Sending the prompt to chatGPT: " + user_message
        reply=reply + ":" + chat(user_message)
        return jsonify({'reply': reply})
    if "homepage" in user_message or "home" in user_message:
        reply = "Sure! Opening Homepage"
        return jsonify({'reply': reply, 'redirect': '/index'}) 
    if "remote lab" in user_message or "rl" in user_message:
        reply = "Sure! Opening Remote Lab"
        return jsonify({'reply': reply, 'redirect': '/remote_lab1_test/'}) 
    if "my sketches" in user_message or "sketches" in user_message:
        reply = "Sure! Opening your sketches"
        print(current_user.get_id())
        user_sketches='/articles_by_author/' + current_user.get_id()
        return jsonify({'reply': reply, 'redirect': user_sketches }) 
    if "activities" in user_message or "active activities" in user_message:
        reply = "Sure! Opening active activities"
        return jsonify({'reply': reply, 'redirect': '/activities_active/'})
    if "submit" in user_message or "submit activity" in user_message:
        reply = "Sure! Submiting Active Activity"
        ActiveActivity = get_ActiveActivity()
        if ActiveActivity==-1:
            reply = " There is no Active Activity."
            return jsonify({'reply': reply})
        else:
            link="/submit_activity/" + str(ActiveActivity)                
            return jsonify({'reply': reply, 'redirect': link })
    if "help" in user_message:
        reply = """Until now I can help you with the following commands:
        status - I open the status activity page
        logout, log out, bye - I log you out
        chat or chatGPT - I open the chatGPT interface
        ask or ask GPT - I ask chatGPT your prompt
        homepage or home - I open the home page
        remote lab or rl - I open the Remote Lab Interface
        activities or active activities - I open the active activities webpage
        submit or submit activity - I submit the active activity
        More actions are coming soon! 
        """
        return jsonify({'reply': reply})        
    reply = f"You said: {user_message}. I'm here to help!"
    return jsonify({'reply': reply})
